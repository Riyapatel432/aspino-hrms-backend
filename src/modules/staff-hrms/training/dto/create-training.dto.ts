...
export class CreateTrainingDto {
  ...
  @IsNotEmpty({ message: 'Training name is required (e.g. GMP Regulatory Compliance).' })
  trainingName: string;

  @IsString({ message: 'Training type must be a string.' })
  @IsIn(['COMPLIANCE', 'TECHNICAL', 'SOFT_SKILLS'], {
    message: 'Training type must be one of: COMPLIANCE, TECHNICAL, or SOFT_SKILLS.',
  })
  trainingType: string;

  @IsDateString({}, { message: 'Completion date must be a valid date (YYYY-MM-DD).' })
